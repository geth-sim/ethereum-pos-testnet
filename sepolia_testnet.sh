...
# NETWORK_DIR=./network
NETWORK_DIR=/data/local-sepolia-testnet
...
GETH_BINARY=/home/minseokk1m/geth-sim/go-ethereum/build/bin/geth
GETH_BOOTNODE_BINARY=/home/minseokk1m/geth-sim/go-ethereum/build/bin/bootnode
PRYSM_CTL_BINARY=/home/minseokk1m/geth-sim/prysm/bazel-bin/cmd/prysmctl/prysmctl_/prysmctl
PRYSM_BEACON_BINARY=/home/minseokk1m/geth-sim/prysm/bazel-bin/cmd/beacon-chain/beacon-chain_/beacon-chain
PRYSM_VALIDATOR_BINARY=/home/minseokk1m/geth-sim/prysm/bazel-bin/cmd/validator/validator_/validator
...
for (( i=0; i<$NUM_NODES; i++ )); do
    ...
    # Set the data directory based on the node index
    if [[ $i -eq 0 ]]; then
        geth_datadir="/data/SnapsyncData"
    else
        geth_datadir="$NODE_DIR/execution"
    fi
    ...
    cp ./sepolia_config.yml $NODE_DIR/consensus/config.yml
    cp ./sepolia_genesis.ssz $NODE_DIR/consensus/genesis.ssz
    cp ./sepolia_genesis.json $NODE_DIR/execution/genesis.json

    # Create the secret keys for this node and other account details
    $GETH_BINARY account new --datadir "$geth_datadir" --password "$geth_pw_file"
    ...
    $GETH_BINARY \
      ...
      --verbosity=4 \
      --syncmode=snap > "$NODE_DIR/logs/geth.log" 2>&1 &

    sleep 5

    # # Start prysm consensus client for this node (initial-sync)
    # if [[ $i -eq 0 ]]; then
    #     prysm_datadir="/data/ConsensusData"
    # else
    #     prysm_datadir="$NODE_DIR/consensus/beacondata"
    # fi

    # Start prysm consensus client for each node (checkpoint-sync)
    if [[ $i -eq 1 ]]; then
        prysm_datadir="$NODE_DIR/consensus/beacondata"
        checkpoint_sync_url="http://localhost:4100" # gRPC gateway of node-0
        additional_options="--checkpoint-sync-url=${checkpoint_sync_url}"
    else
        prysm_datadir="/data/ConsensusData"
        additional_options=""
    fi
    ...
    if [[ -z "${PRYSM_BOOTSTRAP_NODE}" ]]; then
        ...
        PRYSM_BOOTSTRAP_NODE=$(curl -s localhost:4100/eth/v1/node/identity | jq -r '.data.enr')
            echo "PRYSM_BOOTSTRAP_NODE is: $PRYSM_BOOTSTRAP_NODE"
